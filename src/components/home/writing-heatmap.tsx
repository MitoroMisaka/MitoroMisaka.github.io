...
export default function WritingHeatmap({ data }: WritingHeatmapProps) {
  ...
  return (
    <section className="mt-12">
      <h2 className="mb-4 text-sm font-semibold uppercase tracking-[0.2em] text-[var(--fg-soft)]">
        写作热力图
      </h2>
      ...
      <div className="overflow-x-auto pb-2">
        {/* Month labels row */}
        <div
          className="mb-1 ml-7 flex text-xs text-[var(--fg-soft)]"
        ...
        <div className="flex">
          {/* Day labels (y-axis) */}
          <div
            className="mr-1.5 flex flex-col pt-px text-[10px] leading-none text-[var(--fg-soft)]"
            style={{ gap: 'var(--cell-gap, 3px)' }}
          >
            {[0, 1, 2, 3, 4, 5, 6].map((d) => (
              <div
      ...
      {/* Legend */}
      <div className="mt-2 flex items-center gap-1.5 text-[10px] text-[var(--fg-soft)]">
        <span>Less</span>
        {[0, 1, 2, 3].map((level) => (
          <div
            key={level}

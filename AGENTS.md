...
**PRD、TECH、TASKS、ANALYSIS 等计划文档是用户与 AI 之间的合同，不是工作记录。**

- **禁止 subagent 修改任何 docs/ 下的文档**（PRD/TECH/TASKS/ANALYSIS/DEPLOYMENT/WRITING）。
- **禁止未经用户直接指示修改 `[ ]` → `[x]` 标记。** 即使任务已完成，标记权归用户（或用户明确指示的主 agent）。
- **禁止删除、压缩、改写原始子项内容来掩盖未完成的任务。** 原始要求必须逐字保留。
- **禁止篡改需求来匹配实际实现。** 没做的标 ❌，不要改文档让"没做"看起来像"不需要做"。
- 这些文件只能由主 agent 在用户明确指令下修改（如 "更新 TASKS 标记 Phase 1 为完成"），且改动必须是明确的标记操作，不得改写子项内容。

违反以上任意一条即为致命错误，必须回退文档到 git 原始版本。
